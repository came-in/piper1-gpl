...
download_dir = Path("/persistent_storage/piper_voices")
data_dir = [download_dir]
# Download voice if file doesn't exist
model_path = download_dir/"en_US-amy-medium.onnx"
...
# Load voice
default_voice = PiperVoice.load(model_path, use_cuda=False)
    ...
    if voice is None:
        for data_dir in data_dir:
            ...
            if maybe_model_path.exists():
                ...
                voice = PiperVoice.load(maybe_model_path, use_cuda=False)
            ...
            for i, audio_chunk in enumerate(voice.synthesize(text, syn_config)):
                ...
                if i > 0:
                    wav_file.writeframes(
                        bytes(
                            int(
                                voice.config.sample_rate * 0.0 * 2
                            )
                        )
                    )
